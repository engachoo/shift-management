...
- **要事前対応（6文字未満パスワード）**：Firebase Authは6文字以上必須。下記2名は移行前にパスワードを6文字以上へ変更する（方針決定済み。両名とも現在は未使用のため、変更後に本人が使い始める際に新パスワードを通知すればよい）：
  - narita（現 `BL001` → 6文字以上へ変更）
  - nakasima（現 `BL002` → 6文字以上へ変更）
- **他アプリへの影響調査結果**：
